...
function main() {
    (async function main() {
        ...
        async function executeProcess() {
            ...
            try {
                ...
                await executeWithRetry(async () => {
                    const elements = await Promise.all([
                        waitForElement('th .CBX_checkbox_5-117-0'),
                        waitForElement('.BTN_outerWrapperBtn_5-111-0')
                    ]);
                    elements[0].click();
                    elements[1].click();
                });
                ...
                await executeWithRetry(async () => {
                    // 简化版单选框处理逻辑
                    const radioButtons = document.querySelectorAll('tr td:last-child label:last-child');

                    if (radioButtons.length === 0) {
                        console.error('Debug: No radio buttons found using selector');
                ...
                await executeWithRetry(async () => {
                    // 使用更通用的提交按钮选择器
                    const submitButton = await waitForElement('.MDL_inner_5-111-0 button:first-child', OPERATION_TIMEOUT * 2);
                    submitButton.click();
            ...
            // 尝试点击下一页按钮
            try {
                const nextPageButton = await waitForElement('.PGT_next_5-117-0', OPERATION_TIMEOUT);
